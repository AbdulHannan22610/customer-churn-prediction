"""Train and persist the customer churn Random Forest model."""
...
DATA_PATH = PROJECT_ROOT / "data" / "demo_dataset.csv"
...
def build_pipeline(features: pd.DataFrame) -> Pipeline:
    """Build the encoder and classifier from the supplied training features."""
...
def load_training_data() -> tuple[pd.DataFrame, pd.Series]:
    if not DATA_PATH.exists():
        raise FileNotFoundError(
            f"Training data was not found at {DATA_PATH}. "
            "Add data/demo_dataset.csv and run this script again."
        )
    ...
    target = data[TARGET_COLUMN].astype(str).str.strip().str.lower()
    target = target.map({"yes": "Yes", "no": "No"})
    if target.isna().any():
        raise ValueError("The churn column must contain only Yes and No values.")
    if target.nunique() < 2:
